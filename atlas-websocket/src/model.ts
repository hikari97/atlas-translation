...
export interface WebSocketRegistry {
  ...
  resolve<TComponent>(type: string): readonly TComponent[];
}
...
export class InMemoryWebSocketRegistry implements WebSocketRegistry {
  private readonly entriesByType = new Map<string, Set<unknown>>();
  ...
  public register(component: unknown): void {
    const type = this.componentType(component);
    const entries = this.entriesByType.get(type) ?? new Set<unknown>();
    entries.add(component);
    this.entriesByType.set(type, entries);
  }

  public unregister(component: unknown): void {
    const type = this.componentType(component);
  ...
  public resolve<TComponent>(type: string): readonly TComponent[] {
    return Array.from(this.entriesByType.get(type) ?? []).map((component) => component as TComponent);
  }
...
export const createWebSocketRegistry = (
...
): WebSocketRegistry => new InMemoryWebSocketRegistry(metadata, lifecycle);
