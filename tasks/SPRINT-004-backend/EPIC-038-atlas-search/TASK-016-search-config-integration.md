...
# TASK-016 — Config Integration

## Summary

Expose atlas-config integration boundary for search configuration.
...
After this task is complete, Atlas supports the `Config Integration` capability for `atlas-search`.
...
- [x] `Config Integration` is implemented inside `packages/atlas-search`.
...
TASK-017-search-test-utilities.md
