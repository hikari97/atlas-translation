...
Implement provider-neutral query descriptors for term, match, boolean, range, and exists queries.
...
After this task is complete, Atlas supports the `Query DSL` capability for `atlas-search`.

---

# Goal

Implement the smallest production-quality increment needed for **Query DSL**.
...
- [x] `Query DSL` is implemented inside `packages/atlas-search`.
