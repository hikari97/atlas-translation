...
Define facet descriptors and aggregation result contracts.
...
After this task is complete, Atlas supports the `Facets` capability for `atlas-search`.

---

# Goal

Implement the smallest production-quality increment needed for **Facets**.
...
- [x] `Facets` is implemented inside `packages/atlas-search`.
