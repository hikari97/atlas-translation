...
# TASK-010 — Indexing Service

## Summary

Implement add, update, delete, get, and search orchestration over adapter boundary.
...
After this task is complete, Atlas supports the `Indexing Service` capability for `atlas-search`.

---

# Goal

Implement the smallest production-quality increment needed for **Indexing Service**.
...
- [x] `Indexing Service` is implemented inside `packages/atlas-search`.
...
TASK-011-search-batch-indexing.md
