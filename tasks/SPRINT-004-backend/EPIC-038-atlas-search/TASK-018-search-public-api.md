...
After this task is complete, Atlas supports the `Search Public API` capability for `atlas-search`.

---

# Goal

Implement the smallest production-quality increment needed for **Search Public API**.
...
- [x] `Search Public API` is implemented inside `packages/atlas-search`.
