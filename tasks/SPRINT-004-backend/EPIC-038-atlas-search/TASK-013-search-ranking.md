...
Define ranking score metadata and ranking option boundaries.
...
After this task is complete, Atlas supports the `Ranking` capability for `atlas-search`.

---

# Goal

Implement the smallest production-quality increment needed for **Ranking**.
...
- [x] `Ranking` is implemented inside `packages/atlas-search`.
...
TASK-014-search-reindex-plan.md
