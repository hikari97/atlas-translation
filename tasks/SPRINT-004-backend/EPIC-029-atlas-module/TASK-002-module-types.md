...
Define core types for modules, manifests, capabilities, dependencies, lifecycle, and load options.

---

## Goal

Implement module types for `atlas-module` while keeping the package framework-independent, type-safe, deterministic, and isolated from app/runtime-specific infrastructure.
...
- Define module ID, module version, module status, module capability, module manifest, dependency, lifecycle hook, and load option types.
- Create readonly/immutable type shapes where appropriate.
- Model sync and async lifecycle results without forcing implementation decisions.
- Export types from the package public entrypoint.

### Excluded

- Do not implement registry behavior.
- Do not implement graph sorting.
...
packages/atlas-module/test/types.test.ts
```
...
- Core module types are type-safe and documented with concise comments.
- Types are provider-independent and framework-independent.
- Public exports expose only stable type contracts.
- Type tests or runtime tests cover important type helper behavior where applicable.
...
- Run tests for module type helpers if any.
