...
Implement typed module manifests and manifest helper utilities.

---

## Goal

Implement module manifest for `atlas-module` while keeping the package framework-independent, type-safe, deterministic, and isolated from app/runtime-specific infrastructure.
...
- Implement typed module manifest creation and validation helpers.
- Normalize manifest fields such as id, name, version, description, capabilities, dependencies, and compatibility.
- Provide helpers for checking required manifest fields.
- Return explicit diagnostics instead of throwing for validation-only APIs where appropriate.

### Excluded

- Do not implement module registry.
- Do not load modules from disk or network.
- Do not validate external package metadata.
...
packages/atlas-module/test/manifest.test.ts
```
...
- Manifests can be created and normalized deterministically.
- Invalid manifests produce actionable diagnostics.
- No filesystem, package manager, or network assumptions are added.
- Manifest APIs are exported from the package entrypoint.
...
- Test diagnostics for invalid fields.
