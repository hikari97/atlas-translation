...
Implement the registry that stores module definitions and manifest metadata.

---

## Goal

Implement module registry for `atlas-module` while keeping the package framework-independent, type-safe, deterministic, and isolated from app/runtime-specific infrastructure.
...
- Implement an explicit module registry.
- Support registering module definitions.
- Detect duplicate module IDs.
- Expose read-only registry inspection APIs.
- Allow creating isolated registries without global state.

### Excluded

- Do not load modules automatically from disk or network.
- Do not execute lifecycle hooks.
- Do not implement graph sorting in this task.
...
packages/atlas-module/test/registry.test.ts
```
...
- Registry stores module definitions by module ID.
- Duplicate registrations fail with actionable errors or diagnostics.
- Registry inspection does not expose mutable internals.
...
- Test duplicate module ID behavior.
- Test registry isolation.
- Test read-only inspection output.
