...
Expose atlas-config integration boundary for worker configuration.
...
After this task is complete, Atlas supports the `Config Integration` capability for `atlas-worker`.
...
- [x] `Config Integration` is implemented inside `packages/atlas-worker`.
...
TASK-016-worker-diagnostics.md
