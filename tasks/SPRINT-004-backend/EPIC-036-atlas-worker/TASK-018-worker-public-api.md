...
After this task is complete, Atlas supports the `Worker Public API` capability for `atlas-worker`.

---

# Goal

Implement the smallest production-quality increment needed for **Worker Public API**.
...
- [x] `Worker Public API` is implemented inside `packages/atlas-worker`.
