...
Expose worker boundaries for atlas-queue consumer messages.
...
After this task is complete, Atlas supports the `Queue Integration` capability for `atlas-worker`.
...
- [x] `Queue Integration` is implemented inside `packages/atlas-worker`.
...
TASK-014-worker-runtime-integration.md
