...
# TASK-003 — Worker Context

## Summary

Implement execution context contracts for metadata, cancellation, and service access.
...
After this task is complete, Atlas supports the `Worker Context` capability for `atlas-worker`.

---

# Goal

Implement the smallest production-quality increment needed for **Worker Context**.
...
- [x] `Worker Context` is implemented inside `packages/atlas-worker`.
...
TASK-004-worker-job-handler.md
