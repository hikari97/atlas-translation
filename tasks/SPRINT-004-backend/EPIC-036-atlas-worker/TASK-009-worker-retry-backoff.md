...
After this task is complete, Atlas supports the `Retry Backoff` capability for `atlas-worker`.

---

# Goal

Implement the smallest production-quality increment needed for **Retry Backoff**.
...
- [x] `Retry Backoff` is implemented inside `packages/atlas-worker`.
...
TASK-010-worker-cancellation.md
