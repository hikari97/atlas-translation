...
Implement heartbeat contracts and status reporting.
...
After this task is complete, Atlas supports the `Heartbeat` capability for `atlas-worker`.

---

# Goal

Implement the smallest production-quality increment needed for **Heartbeat**.
...
- [x] `Heartbeat` is implemented inside `packages/atlas-worker`.
...
TASK-012-worker-supervision.md
