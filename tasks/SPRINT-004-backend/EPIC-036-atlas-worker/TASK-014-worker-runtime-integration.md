...
Expose atlas-runtime registration hooks for worker capability.
...
After this task is complete, Atlas supports the `Runtime Integration` capability for `atlas-worker`.
...
- [x] `Runtime Integration` is implemented inside `packages/atlas-worker`.
...
TASK-015-worker-config-integration.md
