...
# TASK-008 — Concurrency Limiter

## Summary

Implement deterministic concurrency limiter boundaries.
...
After this task is complete, Atlas supports the `Concurrency Limiter` capability for `atlas-worker`.

---

# Goal

Implement the smallest production-quality increment needed for **Concurrency Limiter**.
...
- [x] `Concurrency Limiter` is implemented inside `packages/atlas-worker`.
...
TASK-009-worker-retry-backoff.md
