---
id: TASK-001
title: Backend Builder Package Foundation
status: Completed
priority: High
story_points: 5
...
# TASK-001 — Backend Builder Package Foundation

## Summary

Create the package foundation for atlas-backend-builder.

This task belongs to **EPIC-040-atlas-backend-builder** and must be implemented only inside `atlas-backend-builder`.

---

# Capability

After this task is complete, Atlas supports the `Backend Builder Package Foundation` capability for `atlas-backend-builder`.

---

# Goal

Implement the smallest production-quality increment needed for **Backend Builder Package Foundation**.
...
`atlas-backend-builder` is part of SPRINT-004-backend and must follow package boundaries, public API rules, and task scope.
...
- Keep code inside `packages/atlas-backend-builder`.
...
```txt
packages/atlas-backend-builder/src/**
packages/atlas-backend-builder/tests/**
packages/atlas-backend-builder/package.json
packages/atlas-backend-builder/tsconfig.json
```
...
- If `packages/{package}` does not exist, create it.
- Do not edit root workspace files unless the repository already requires it and the task cannot be completed; if so, stop and report.
...
```txt
Any path outside packages/atlas-backend-builder/**
apps/**
plugins/**
docs/**
tasks/**
...
- [x] `Backend Builder Package Foundation` is implemented inside `packages/atlas-backend-builder`.
...
```bash
pnpm --filter atlas-backend-builder test
pnpm --filter atlas-backend-builder typecheck
pnpm --filter atlas-backend-builder build
```
...
TASK-002-backend-builder-types.md
