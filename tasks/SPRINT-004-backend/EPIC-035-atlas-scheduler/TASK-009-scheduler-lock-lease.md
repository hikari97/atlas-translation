...
Define lock and lease contracts for distributed-safe scheduling.
...
After this task is complete, Atlas supports the `Lock Lease` capability for `atlas-scheduler`.

---

# Goal

Implement the smallest production-quality increment needed for **Lock Lease**.
...
- [x] `Lock Lease` is implemented inside `packages/atlas-scheduler`.
...
TASK-010-scheduler-missed-runs.md
