...
# TASK-014 — Worker Integration

## Summary

Expose scheduled execution boundaries to atlas-worker public API.
...
After this task is complete, Atlas supports the `Worker Integration` capability for `atlas-scheduler`.

---

# Goal

Implement the smallest production-quality increment needed for **Worker Integration**.
...
- [x] `Worker Integration` is implemented inside `packages/atlas-scheduler`.
...
TASK-015-scheduler-config-integration.md
