...
After this task is complete, Atlas supports the `Scheduler Public API` capability for `atlas-scheduler`.

---

# Goal

Implement the smallest production-quality increment needed for **Scheduler Public API**.
...
- [x] `Scheduler Public API` is implemented inside `packages/atlas-scheduler`.
