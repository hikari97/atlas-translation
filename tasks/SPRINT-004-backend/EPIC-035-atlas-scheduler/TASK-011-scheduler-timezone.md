...
Define timezone-aware metadata and validation boundaries.
...
After this task is complete, Atlas supports the `Timezone` capability for `atlas-scheduler`.

---

# Goal

Implement the smallest production-quality increment needed for **Timezone**.
...
- [x] `Timezone` is implemented inside `packages/atlas-scheduler`.
...
TASK-012-scheduler-persistence-boundary.md
