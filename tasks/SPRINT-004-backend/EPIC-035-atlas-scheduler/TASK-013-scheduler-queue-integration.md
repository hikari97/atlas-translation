...
Expose dispatch boundaries to atlas-queue public API.
...
After this task is complete, Atlas supports the `Queue Integration` capability for `atlas-scheduler`.
...
- [x] `Queue Integration` is implemented inside `packages/atlas-scheduler`.
...
TASK-014-scheduler-worker-integration.md
