...
# TASK-014 — Idempotency

## Summary

Implement idempotency key contracts and duplicate-handling boundaries.
...
After this task is complete, Atlas supports the `Idempotency` capability for `atlas-queue`.

---

# Goal

Implement the smallest production-quality increment needed for **Idempotency**.
...
- [x] `Idempotency` is implemented inside `packages/atlas-queue`.
...
TASK-015-queue-config-integration.md
