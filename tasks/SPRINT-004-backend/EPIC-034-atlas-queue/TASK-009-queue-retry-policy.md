...
# TASK-009 — Retry Policy

## Summary

Implement retry metadata, retry decisions, and backoff descriptors.
...
After this task is complete, Atlas supports the `Retry Policy` capability for `atlas-queue`.

---

# Goal

Implement the smallest production-quality increment needed for **Retry Policy**.
...
- [x] `Retry Policy` is implemented inside `packages/atlas-queue`.
