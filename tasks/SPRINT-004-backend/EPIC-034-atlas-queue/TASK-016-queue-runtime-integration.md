...
Expose atlas-runtime registration hooks for queue capability.
...
After this task is complete, Atlas supports the `Runtime Integration` capability for `atlas-queue`.
...
- [x] `Runtime Integration` is implemented inside `packages/atlas-queue`.
...
TASK-017-queue-test-utilities.md
