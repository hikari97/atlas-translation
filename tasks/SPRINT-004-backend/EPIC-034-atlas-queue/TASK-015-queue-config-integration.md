...
Expose atlas-config integration boundary for queue configuration.
...
After this task is complete, Atlas supports the `Config Integration` capability for `atlas-queue`.
...
- [x] `Config Integration` is implemented inside `packages/atlas-queue`.
...
TASK-016-queue-runtime-integration.md
