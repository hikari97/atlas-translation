...
# TASK-004 — Queue Adapter Contract

## Summary

Define queue adapter operations without provider coupling.
...
After this task is complete, Atlas supports the `Queue Adapter Contract` capability for `atlas-queue`.

---

# Goal

Implement the smallest production-quality increment needed for **Queue Adapter Contract**.
...
- [x] `Queue Adapter Contract` is implemented inside `packages/atlas-queue`.
...
TASK-005-queue-memory-adapter.md
