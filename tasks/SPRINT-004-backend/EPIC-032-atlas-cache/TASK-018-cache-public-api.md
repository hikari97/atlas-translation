...
After this task is complete, Atlas supports the `Cache Public API` capability for `atlas-cache`.

---

# Goal

Implement the smallest production-quality increment needed for **Cache Public API**.
...
- [x] `Cache Public API` is implemented inside `packages/atlas-cache`.
