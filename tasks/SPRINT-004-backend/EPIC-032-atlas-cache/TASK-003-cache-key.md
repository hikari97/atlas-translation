...
Implement key normalization, namespacing, and key validation helpers.
...
After this task is complete, Atlas supports the `Cache Key` capability for `atlas-cache`.

---

# Goal

Implement the smallest production-quality increment needed for **Cache Key**.
...
- [x] `Cache Key` is implemented inside `packages/atlas-cache`.
