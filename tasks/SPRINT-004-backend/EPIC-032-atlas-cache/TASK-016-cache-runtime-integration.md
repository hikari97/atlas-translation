...
Expose atlas-runtime registration hooks for cache capability.
...
After this task is complete, Atlas supports the `Runtime Integration` capability for `atlas-cache`.
...
- [x] `Runtime Integration` is implemented inside `packages/atlas-cache`.
...
TASK-017-cache-test-utilities.md
