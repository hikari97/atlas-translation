...
# TASK-004 — Cache Entry

## Summary

Define cache entry metadata, timestamps, expiration, and version metadata.
...
After this task is complete, Atlas supports the `Cache Entry` capability for `atlas-cache`.

---

# Goal

Implement the smallest production-quality increment needed for **Cache Entry**.
...
- [x] `Cache Entry` is implemented inside `packages/atlas-cache`.
...
TASK-005-cache-store-contract.md
