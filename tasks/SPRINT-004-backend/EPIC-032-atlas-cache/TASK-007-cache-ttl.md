...
# TASK-007 — TTL Expiration

## Summary

Implement TTL calculation, expiration checks, and stale behavior helpers.
...
After this task is complete, Atlas supports the `TTL Expiration` capability for `atlas-cache`.

---

# Goal

Implement the smallest production-quality increment needed for **TTL Expiration**.
...
- [x] `TTL Expiration` is implemented inside `packages/atlas-cache`.
