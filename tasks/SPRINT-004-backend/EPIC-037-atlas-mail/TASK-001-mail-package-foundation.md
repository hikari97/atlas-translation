...
# TASK-001 — Mail Package Foundation

## Summary

Create the package foundation for atlas-mail.

This task belongs to **EPIC-037-atlas-mail** and must be implemented only inside `atlas-mail`.

---

# Capability

After this task is complete, Atlas supports the `Mail Package Foundation` capability for `atlas-mail`.

---

# Goal

Implement the smallest production-quality increment needed for **Mail Package Foundation**.
...
`atlas-mail` is part of SPRINT-004-backend and must follow package boundaries, public API rules, and task scope.
...
- Keep code inside `packages/atlas-mail`.
...
```txt
packages/atlas-mail/src/**
packages/atlas-mail/tests/**
packages/atlas-mail/package.json
packages/atlas-mail/tsconfig.json
```
...
```txt
Any path outside packages/atlas-mail/**
apps/**
plugins/**
docs/**
tasks/**
...
- [x] `Mail Package Foundation` is implemented inside `packages/atlas-mail`.
...
```bash
pnpm --filter atlas-mail test
pnpm --filter atlas-mail typecheck
pnpm --filter atlas-mail build
```
