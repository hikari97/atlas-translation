---
id: TASK-004
title: Database Client Contract
status: Completed
priority: High
story_points: 5
...
Define the database client boundary used by higher-level packages.
...
After this task is complete, Atlas supports the `Database Client Contract` capability for `atlas-database`.

---

# Goal

Implement the smallest production-quality increment needed for **Database Client Contract**.
...
- [x] `Database Client Contract` is implemented inside `packages/atlas-database`.
...
TASK-005-database-connection-manager.md
