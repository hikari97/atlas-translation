---
id: TASK-007
title: Database Transaction Runner
status: Completed
priority: High
story_points: 5
...
Implement transaction orchestration helpers around provider-neutral callbacks.
...
After this task is complete, Atlas supports the `Database Transaction Runner` capability for `atlas-database`.

---

# Goal

Implement the smallest production-quality increment needed for **Database Transaction Runner**.
...
- [x] `Database Transaction Runner` is implemented inside `packages/atlas-database`.
...
TASK-008-database-query-result.md
