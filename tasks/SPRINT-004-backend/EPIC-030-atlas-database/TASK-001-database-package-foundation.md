---
id: TASK-001
title: Database Package Foundation
status: Completed
priority: High
story_points: 5
...
# TASK-001 — Database Package Foundation

## Summary

Create the package foundation for atlas-database.

This task belongs to **EPIC-030-atlas-database** and must be implemented only inside `atlas-database`.

---

# Capability

After this task is complete, Atlas supports the `Database Package Foundation` capability for `atlas-database`.

---

# Goal

Implement the smallest production-quality increment needed for **Database Package Foundation**.
...
`atlas-database` is part of SPRINT-004-backend and must follow package boundaries, public API rules, and task scope.
...
- Keep code inside `packages/atlas-database`.
...
```txt
packages/atlas-database/src/**
packages/atlas-database/tests/**
packages/atlas-database/package.json
packages/atlas-database/tsconfig.json
```
...
```txt
Any path outside packages/atlas-database/**
apps/**
plugins/**
docs/**
tasks/**
...
- [x] `Database Package Foundation` is implemented inside `packages/atlas-database`.
...
pnpm --filter atlas-database typecheck
