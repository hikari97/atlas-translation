...
Define connection configuration contracts without reading process.env directly.
...
After this task is complete, Atlas supports the `Database Connection Config` capability for `atlas-database`.

---

# Goal

Implement the smallest production-quality increment needed for **Database Connection Config**.
...
- [x] `Database Connection Config` is implemented inside `packages/atlas-database`.
...
TASK-004-database-client-contract.md
