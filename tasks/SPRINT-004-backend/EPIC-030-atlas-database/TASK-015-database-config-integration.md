...
Expose atlas-config integration boundary for normalized database configuration.
...
After this task is complete, Atlas supports the `Config Integration` capability for `atlas-database`.
...
- [x] `Config Integration` is implemented inside `packages/atlas-database`.
...
TASK-016-database-runtime-integration.md
