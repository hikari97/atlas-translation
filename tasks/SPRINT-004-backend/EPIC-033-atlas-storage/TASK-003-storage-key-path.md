...
# TASK-003 — Storage Key Path

## Summary

Implement key/path normalization and safe path validation helpers.
...
After this task is complete, Atlas supports the `Storage Key Path` capability for `atlas-storage`.

---

# Goal

Implement the smallest production-quality increment needed for **Storage Key Path**.
...
- [x] `Storage Key Path` is implemented inside `packages/atlas-storage`.
