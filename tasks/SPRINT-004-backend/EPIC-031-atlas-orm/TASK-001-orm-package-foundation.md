...
# TASK-001 — ORM Package Foundation

## Summary

Create the package foundation for atlas-orm.

This task belongs to **EPIC-031-atlas-orm** and must be implemented only inside `atlas-orm`.

---

# Capability

After this task is complete, Atlas supports the `ORM Package Foundation` capability for `atlas-orm`.

---

# Goal

Implement the smallest production-quality increment needed for **ORM Package Foundation**.
...
`atlas-orm` is part of SPRINT-004-backend and must follow package boundaries, public API rules, and task scope.
...
- Keep code inside `packages/atlas-orm`.
...
```txt
packages/atlas-orm/src/**
packages/atlas-orm/tests/**
packages/atlas-orm/package.json
packages/atlas-orm/tsconfig.json
```
...
```txt
Any path outside packages/atlas-orm/**
apps/**
plugins/**
docs/**
tasks/**
...
- [x] `ORM Package Foundation` is implemented inside `packages/atlas-orm`.
...
```bash
pnpm --filter atlas-orm test
pnpm --filter atlas-orm typecheck
pnpm --filter atlas-orm build
```
